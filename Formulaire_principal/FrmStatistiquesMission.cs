    ...
    public partial class FrmStatistiquesMission : Form
    {
        ...
        public void GetInformateursMoinsPayes(string planete, int numero)
        {
            ...
            try
            {
                ...
                // Utilisation de paramètres pour sécuriser la saisie 
        ...
        private void GetRentabiliteMission(string p, int n)
        {
            ...
            string sql = @"SELECT @p || '-' || @n AS Mission, 
                   (SELECT SUM(qteDataBaz) FROM Negocier WHERE nomPlanete=@p AND numeroMission=@n) AS 'DataBaz (kg)',
