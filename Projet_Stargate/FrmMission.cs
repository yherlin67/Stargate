...
namespace Projet_Stargate
{
    ...
    public partial class FrmMission : Form
    {
        ...
        public FrmMission()
        {
            ...
            lblChoixChef.ForeColor = SystemColors.ControlDark;
        ...
        {
            string sqlCount1 = @"SELECT COUNT(*) FROM Membre me
            ...
            string sqlCount2 = $@"SELECT COUNT(*) FROM Membre me
            JOIN Militaire mi ON me.matricule = mi.matriculeMembre
            WHERE me.matricule != '{cboChef.SelectedValue}' AND me.matricule LIKE 'M%'
            AND me.matricule NOT IN
                (SELECT matriculeMembre FROM Composer co
                JOIN Mission mi2 ON co.nomPlanete = mi2.nomPlanete 
                AND co.numeroMission = mi2.numero
                WHERE dateDepart <= CURRENT_DATE AND dateRetour >= CURRENT_DATE)";
            ...
            else
            {
                ...
                try
                {
                    int nbCivils = Convert.ToInt32(new SQLiteCommand(sqlCount1, co).ExecuteScalar());
                    int nbMilitaires = Convert.ToInt32(new SQLiteCommand(sqlCount2, co).ExecuteScalar());
                    totalMembres = nbCivils + nbMilitaires;
                }
                catch (SQLiteException err)
                {
                    MessageBox.Show(err.Message);
                ...
                if (int.Parse(txtnbMembres.Text) > totalMembres)
                {
                    MessageBox.Show($"Impossible de sélectionner autant de membres !\nNombre maximum de membres disponibles : {totalMembres}");
                }
